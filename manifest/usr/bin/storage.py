...
class S3Storage:
    def __init__(self):
        ...
        self.config["signature_version"] = os.getenv("S3_SIGNATURE_VERSION", "s3v4")
        ...
        if not self.config["access_key"] or not self.config["secret_key"]:
            ...
            ) = self.get_sts_creds(self.config["host"], _ldap_user, _ldap_pass)

        for i in self.config.keys():
            if self.config[i] is None:
                raise OSError(
                    2,
                    f'missing required config key or environment variable "{i}" proceed',
                )
    ...
    def upload(self, local_path):
        ...
        storage.Bucket(self.config["bucket"]).upload_file(
